import React, { useEffect, useState } from "react";
import companySettingsWorker from "../../../../../worklayer/companySettingsWorker";
import ActiveButton from "../../../../dynamicComponents/buttons/ActiveButton";
import DefaultIcon from "../../../../dynamicComponents/Icons/DefaultIcon";

//AlertMessage
import alertMessage from "../../../../../hooks/alertMessage";
import alertError from "../../../../../hooks/alertError";

const CrmIntegrations = ({ setInitialContent }) => {
  //State
  const [isActiveButton, setIsActiveButton] = useState(false);
  const [hubSpot, setHubSpot] = useState();

  //Return InitialContent Menu
  const returnInitialSettingsMenu = () => {
    setInitialContent("initialSettings");
  };

  useEffect(() => {
    companySettingsWorker
      .getHubSpotIntegration()
      .then((res) => {
        const isHubSpotActive = res.data.isHubSpotActive;

        setHubSpot(res.data);
        if (isHubSpotActive === true) {
          setIsActiveButton(true);
        }
      })
      .catch((err) => {
        console.log(err);
      });
  }, []);

  const handleActiveButtonRotate = () => {
    setIsActiveButton(!isActiveButton);

    let obj = {
      id: 1,
      hubSpotOwnerId: "0",
      hubSpotToken: "string",
      isHubSpotActive: isActiveButton,
    };

    companySettingsWorker
      .updataHubSpotIntegration(obj)
      .then((res) => {
        alertMessage({
          data: [{ user: "CRM Status Updated" }],
        });
      })
      .catch((err) => {
        alertError({
          data: [{ error: "CRM Status Not Updated" }],
        });
      });
  };

  return (
    <>
      <div className="row d-flex m-2 border-bottom ">
        <DefaultIcon
          layoutStyles={{
            width: "1rem",
            marginRight: "1rem",
            cursor: "pointer",
          }}
          onClick={returnInitialSettingsMenu}
          className="bi bi-arrow-left-circle"
        />
        <span className="addEditUserTitle">CRM Integrations</span>
      </div>
      <div className="crmLayout">
        <div className="row">
          <div className="col-sm-8 mt-4">
            <img src="data:image/png;base64," />
            <span style={{ marginLeft: ".4rem" }}>IsHubspotCrmIntegrated</span>
          </div>
          <div className="col-sm-4 mt-4">
            <div
              style={{
                backgroundColor: isActiveButton === false ? "grey" : "#4a8869",
              }}
              onClick={handleActiveButtonRotate}
              className="activeButtonLayout"
            >
              <div
                style={{
                  marginLeft: isActiveButton === false ? "1.5rem" : "0px",
                }}
                className="activeButtonOnnOff"
              ></div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default CrmIntegrations;
